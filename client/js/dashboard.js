...
function generateTopTeams() {
  ...
  if (!activeTournament || !activeTournament.groups || activeTournament.playoffSpots <= 0) {
      ...
      <div class="divide-y divide-gray-800">
        ${group.teams
          ...
          <div class="px-3 py-2 flex items-center justify-between ${
            index < activeTournament.playoffSpots ? "bg-green-900 bg-opacity-20" : ""
          }">
            <span class="text-sm font-medium ${
              index < activeTournament.playoffSpots ? "text-green-400" : "text-white"
            }">${team.name}</span>
            <span class="text-xs font-bold ${
              index < activeTournament.playoffSpots ? "text-green-400" : "text-gray-300"
            }">${team.points || 0}</span>
          </div>
        `
          )
...
window.navigateToMatch = function (groupIndex, matchIndex) {
  ...
  setTimeout(() => {
    const matchElement = document.getElementById(`${groupIndex}-m${matchIndex}-home`);
    if (matchElement) {
      matchElement.scrollIntoView({ behavior: "smooth", block: "center" });
      matchElement.closest("tr").classList.add("bg-accent", "bg-opacity-20");
      setTimeout(() => {
        matchElement.closest("tr").classList.remove("bg-accent", "bg-opacity-20");
      }, 2000);
    }
  }, 500);
};
